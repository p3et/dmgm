...
public abstract class SubgraphMiningBase<G extends WithGraph, S>
  ...
  public Long apply(Long inputCollectionId) {
    ...
    Map<DFSCode, List<WithDFSEmbedding>> patternEmbeddings = getParallelizableStream(graphIndex.values())
    ...
    List<Pair<DFSCode, S>> frequentPatterns = addSupportAndFilter(patternEmbeddings, minSupportAbsolute, graphIndex, parallel)
      .collect(Collectors.toList());
    ...
    while (!frequentPatterns.isEmpty() && edgeCount < maxEdgeCount) {
      Map<DFSCode, List<WithDFSEmbedding>> finalPatternEmbeddings = patternEmbeddings;
      ...
      frequentPatterns = addSupportAndFilter(patternEmbeddings, minSupportAbsolute, graphIndex, false)
...
//        .peek(p -> System.out.println("\t" + p.getKey().toString(database) + " : " + p.getValue() + " - " ))
      ...
      graphIds = ArrayUtils.addAll(graphIds, output(frequentPatterns, patternEmbeddings, graphIndex, minSupportAbsolute));
   ...
   */
  private Map<DFSCode, List<WithDFSEmbedding>> growPatternChildren(
    DFSCode parent, List<WithDFSEmbedding> embeddings, Map<Long, G> graphIndex) {
    ...
    List<Pair<DFSCode, WithDFSEmbedding>> children = Lists.newArrayList();

    BiConsumer<G, WithDFSEmbedding> joinFunction =
