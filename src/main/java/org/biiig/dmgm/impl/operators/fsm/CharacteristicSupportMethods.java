...
public interface CharacteristicSupportMethods<G extends WithGraph & WithCategories>
  extends SubgraphMiningSupportMethods<G, Map<Integer, Long>>, DMGMOperator {
  ...
  @Override
  default <K, E extends WithDFSEmbedding> Stream<Pair<K, Map<Integer, Long>>> addSupportAndFilter(
    Map<K, List<E>> patternEmbeddings, Map<Integer, Long> minSupportAbsolute, Map<Long, G> graphIndex, boolean parallel) {
  ...
  default <K, E extends WithDFSEmbedding> Pair<K, Map<Integer, Long>> addSupport(Map.Entry<K, List<E>> entry, Map<Long, G> graphIndex) {

    Map<Integer, Long> support = entry.getValue()
      .stream()
      .map(WithDFSEmbedding::getEmbedding)
