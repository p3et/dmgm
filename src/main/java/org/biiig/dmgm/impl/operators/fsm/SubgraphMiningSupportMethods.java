...
/**
 * The methods of a subgraph mining algorithm that are related to support counting and support based filtering.
 *
 * @param <S> support type
 */
public interface SubgraphMiningSupportMethods<G extends WithGraph, S> {

  /**
   * Calculate the absolute min support threshold regarding to the mining variant, i.e., a single value or a map: category -> value
   *
   ...
   */
  <K, E extends WithDFSEmbedding> Stream<Pair<K, S>> addSupportAndFilter(
    Map<K, List<E>> patternEmbeddings, S minSupportAbsolute, Map<Long, G> graphIndex, boolean parallel);
