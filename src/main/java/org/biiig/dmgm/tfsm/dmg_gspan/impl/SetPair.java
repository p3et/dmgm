...
public class SetPair extends DirectedGraphBase {
  private int[] sourceTargetMux;

  SetPair(int vertexCount, int edgeCount) {
  ...
  private int[] getIncidentEdgeIds(int vertexId, boolean outgoing) {
    ...
    int incidentEdgeCount = edgeIds.length;
    if (incidentEdgeCount != edgeCount) {
      if (incidentEdgeCount == 0) {
        edgeIds = new int[0];
      } else {
